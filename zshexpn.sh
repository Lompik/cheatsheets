# glossary
# zsh support {history,alias, parameter, artihmetic, brace, filename} expansion
## history expansion !, !!,!n, !-n, !str, !?str
## history expansion modifiers are each  preceded  by  a  `:'
## a (absolute path), h[digits] (remove trailing pathname component), Q (unquote), q (quote), s/l/r[/:G], r (root)
# glob qualifiers: If a `:' appears in a qualifier list, the remainder of the expression in parenthesis is interpreted as a modifier
## parameter expansion ${(here)blhablha}: @ (quote & sep), u (unique),f (ps:\n:), F(pj:\n:), q(quote), Q(unquote)
## brace {01..09}
## globbing flags: (#here): i (case insen), e (end), s (start)
# A plain old glob
print -l *.txt
print -l **/*.txt
