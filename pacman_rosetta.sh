...
apt-get upgrade (press n to only see the list)
...
for pacman dependency level, use pacman -Dk; for shared library level, use findbrokenpkgsAUR or lddd (from devtools)
...
# #+begin_src python
# import pandas as pd
# dfs = pd.read_html("https://wiki.archlinux.org/index.php/Pacman/Rosetta#Querying_package_dependencies")
# DISTS = {
#     "arch": "Arch",
#     "rh":"Red Hat/Fedora",
...
#     print(set(["Action"] +list(DISTS.values())))
#     if set(["Action"] +list(DISTS.values())) != set(df.columns):
#         continue
#     for (idx, line) in df.iterrows():
#         print("# ", line["Action"])
#         print("\n".join(line[list(DISTS.values())].fillna("")))
#         print("\n")
# #+end_src
